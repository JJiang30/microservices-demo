...
app.get('/order/:id', (req, res) => {
  Order.findById(req.params.id).then((order) => {
    ...
    if (order) { // http://host.docker.internal:5000
      axios.get(`http://host.docker.internal:5000/customer/${order.customerID}`).then((response) => {
        let orderObject = { CustomerName: response.data.name, BookTitle: '' }
        // http://localhost:3000/book/${order.bookID}
        axios.get(`http://host.docker.internal:3000/book/${order.bookID}`).then((response) => {
